...
void doPhysicsTick()
{
    ...
    else if (temp_state.climbing_direction == DOWN)
    {
        ...
        {
            // movement crosses tile boundary so will require a check against what's below, and also maybe detach pack.
    ...
    // draw lasers
    {
        ...
        for (int tile_index = 0; tile_index < 2 * level_dim.x*level_dim.y*level_dim.z; tile_index += 2)
        {
            ...
            else
            {
                ...
                else drawAsset(getCube3DId(draw_tile), CUBE_3D, intCoordsToNorm(bufferIndexToCoords(tile_index)), DEFAULT_SCALE, directionToQuaternion(world_state.buffer[tile_index + 1]), (Vec4){0}, (Vec4){0}, (Vec4){0});
            }
        }
